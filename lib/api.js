...
export async function getWeather(location = 'Chicago') {
  // Use static JSON file for static exports/builds (vercel, etc)
  if (process.env.NODE_ENV === 'production') {
    return fetchAPI('/api/weather-temp.json');
  }
  return fetchAPI(`/api/weather?location=${encodeURIComponent(location)}`);
}

/**
 * Fetch weather data for a specific zip code
 * @param {string} zipCode - The zip code to get weather for
 * @returns {Promise<Object>} - Weather data
 */
export async function getWeatherByZip(zipCode = '92054') {
  // Use static JSON file for static exports/builds (vercel, etc)
  if (process.env.NODE_ENV === 'production') {
    return fetchAPI('/api/weather-temp.json');
  }
  return fetchAPI(`/api/weather?zip=${encodeURIComponent(zipCode)}`);
}
